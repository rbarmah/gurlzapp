...
export function getCyclePredictions(userData: UserData): PredictionResult {
  ...
  // If no period data is provided, predict based on default cycle length
  if (periods.length === 0) {
    const defaultCycleLength = 28; // Common average cycle length for women
    const defaultPeriodDuration = 5; // Common average period duration

    const today = new Date();
    const predictedNextPeriodStart = new Date(today);
    predictedNextPeriodStart.setDate(today.getDate() + defaultCycleLength);

    const predictedNextPeriodEnd = new Date(predictedNextPeriodStart);
    predictedNextPeriodEnd.setDate(predictedNextPeriodStart.getDate() + defaultPeriodDuration - 1);
    ...
    return {
      message: "Predictions computed using default values.",
      predictions: {
